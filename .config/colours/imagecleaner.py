...
def process_image(filepath):
    ...
    with im.open(filepath) as current_image:
        ratio = current_image.width / current_image.height
        target_ratio = MAX_WIDTH / MAX_HEIGHT
        if (ratio < target_ratio and current_image.width > MAX_WIDTH):
        ...
        elif (current_image.height > MAX_HEIGHT):
            print("> height is " + str(current_image.height) + ", resizing to " + str(MAX_HEIGHT) + "px")
            height_percent = MAX_HEIGHT / float(current_image.height)
            new_width = int((float(current_image.width) * float(height_percent)))
            current_image = current_image.resize((new_width, MAX_HEIGHT), im.Resampling.LANCZOS)
